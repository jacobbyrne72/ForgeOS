...
CRITICAL EPISTEMIC RULE, mirrored from `hive.knowledge.claims`: a trait is a
...
gate `hive/knowledge/claims.py` enforces for outside claims — this harness's
...
# ~4 chars/token, the same rough approximation hive.prompts.prefix uses to
...
def record_outcome(model_ref: str, tendency: Tendency, *, confirmed: bool) -> Trait | None:
    ...
    if confirmed:
        ...
        existing.source = existing.source or "hive ledger"
        return existing

...
hive is always the ACP *client* side of the connection: it spawns a vendor
...
class _HiveClient:
    """hive's ACP `Client` half of the connection.

    hive is a headless harness, not an editor, so it declines the
...
class ACPAdapter(WorkerAdapter):
    ...
    async def start(self, task_id: str, cwd: str, model_profile: str) -> str:
        ...
        # operator's own environment rather than hive constructing one, so
        # hive never reads or copies whatever credential the CLI needs.
        ...
        init = await conn.initialize(
            ...
            client_info=acp.schema.Implementation(name="hive", title="hive", version="0.1.0"),
        )

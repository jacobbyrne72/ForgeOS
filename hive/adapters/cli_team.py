...
- There are *two* different `team.ts`-shaped files in the plugin:
  ...
  *not* the role vocabulary (`executor`, `architect`, `explore`, ...) this
  ...
  over it: reconciling hive's role-shaped `agent_type` with omc's
...
def _slugify_team_name(task_id: str) -> str:
    ...
    and ending alnum, <= 50 chars. A random suffix keeps two hive tasks that
    ...
    return f"hive-{base}-{suffix}"


@dataclass
class _Session:
    """Live state for one hive-side omc team session.
...
class OMCTeamAdapter(WorkerAdapter):
    ...
    (here, one agent type per hive session) always comes from whatever the
    ...
    def capabilities(self) -> WorkerCapabilities:
        return WorkerCapabilities(
            ...
            resumable_sessions=True,  # job_id outlives any one hive session; checkpoint/resume rehydrate it
    ...
    async def start(self, task_id: str, cwd: str, model_profile: str) -> str:
        ...
        # opens the local bookkeeping a hive session_id needs. `team_name`

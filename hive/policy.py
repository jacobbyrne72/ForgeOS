...
# Optional external digest tool. Detected, never assumed — hive must be useful on a
# machine that has never heard of codex-brain, which is every machine but one.
DIGEST_TOOL_ENV = "HIVE_DIGEST_TOOL"
PACK_TOOL_ENV = "HIVE_PACK_TOOL"
...
def ripper_command(path: str, budget: int = 5000) -> str:
    ...
    Falls back to hive's own bounded-read advice when no external digest tool is

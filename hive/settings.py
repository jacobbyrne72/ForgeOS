...
1. **hive never stores a secret.** Providers reference an env var *name* or delegate
   ...
   hive shells out to that flow and afterwards can only observe *whether* a
...
SETTINGS_PATH = Path(os.path.expanduser("~/.hive/settings.json"))
...
class Provider(BaseModel):
    ...
    def authenticated(self) -> bool:
        ...
        is as much as hive can honestly claim without running the CLI.
        """
...
def default_settings() -> Settings:
    ...
    for name, command, auth, login, caps in _CLI_DEFAULTS:
        providers[name] = Provider(
            ...
            login_command=login,
            capabilities=caps,
            notes="Vendor CLI owns its own credentials; hive never reads the token.",
        )

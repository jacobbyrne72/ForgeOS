...
Plain markdown on purpose. If Obsidian is installed, hive uses an existing vault so
...
# Where Obsidian records the vaults a user already has. Reading it means hive can
...
def resolve_vault_path(configured: str | None = None) -> tuple[Path, str, bool]:
    ...
    Order of preference, most explicit first:
      ...
      2. $HIVE_VAULT               — the operator said so, persistently
      3. an existing Obsidian vault — put notes where they will actually be read
      4. ~/.hive/vault             — a plain folder that needs no extra software
    """
    if configured:
    ...
    for vp in obsidian_vaults():
        # A subfolder, never the vault root — hive must not scatter files through
        # notes a human has organised.
        return vp / "hive", "obsidian", True

    return Path(os.path.expanduser("~/.hive/vault")), "default", False
...
class Vault:
    ...
    def read_page(self, path: str | Path) -> str:
        ...
        resolved = p.resolve()
        # Containment, not a string prefix: "~/.hive/vault_backup" starts with
        # "~/.hive/vault" and would have passed a startswith() check.

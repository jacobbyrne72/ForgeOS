...
# Where a models.dev-shaped catalog might already exist. All optional: hive works
# with an empty catalog, it just cannot price a call it has never heard of. Set
# HIVE_MODEL_CATALOG to point at your own copy.
CATALOG_ENV = "HIVE_MODEL_CATALOG"

DEFAULT_CATALOG_PATHS = tuple(
    Path(os.path.expanduser(p))
    for p in (
        "~/.hive/models_dev_cache.json",
        "~/.cache/hive/models_dev_cache.json",
...
class Catalog:
    ...
    def discover(cls, extra_paths: tuple[Path, ...] = ()) -> Catalog:
        ...
        An empty catalog must never be a crash — hive has to work on a machine

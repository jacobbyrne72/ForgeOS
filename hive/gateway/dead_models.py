...
granularity hive actually has -- `(transport_name, model_ref)`, not a whole
provider connection:
...
`hive/gateway/client.py` is the only writer today: it calls `mark_dead`
...
transport's own health, and conflating the two is exactly the bug hive
...
Backed by its own SQLite table opened through `hive/_sqlite.py`'s guarded

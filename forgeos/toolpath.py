"""Trusted binary resolution. One resolver, used everywhere forgeos spawns a process.
...
forgeos is pointed at repositories it does not trust. A hostile repo that ships
`ruff.bat`, `semgrep.exe`, or `node.bat` in its root gets that file executed by
forgeos's own security scanner, as the operator, with every credential on the machine.
No model is involved and no prompt injection is required — the attacker only needs
the operator to run forgeos against the repo.
...
class ToolNotTrusted(RuntimeError):
    """A binary was found, but not somewhere forgeos is willing to execute from."""

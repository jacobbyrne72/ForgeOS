...
wild. So this module draws a hard line:
    ...
    * Every recommendation lands in the existing `forgeos.knowledge.claims`
...
`Transport` protocol in `forgeos/gateway/client.py`) so tests always inject a
...
# ---------------------------------------------------------------------------
# fetching — injectable, mirrors forgeos/gateway/client.py's Transport protocol
# ---------------------------------------------------------------------------
...
class HttpFetcher:
    ...
    def get_json(self, url: str, *, params: dict | None = None, headers: dict | None = None) -> object:
        hdrs = {"Accept": "application/vnd.github+json", "User-Agent": "forgeos-scout/0.1"}
        if headers:
            hdrs.update(headers)
...
def recommend(claim_store: ClaimStore, candidates: Sequence[Candidate], job_id: str) -> list[Claim]:
    ...
    exercised through the existing gate in `forgeos/knowledge/claims.py`.

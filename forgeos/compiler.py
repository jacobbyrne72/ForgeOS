...
@dataclass
class Symbol:
    name: str; kind: str; path: str; start_line: int; end_line: int
    language: str; docstring: str = ""; signature: str = ""
    @property
    def range(self) -> str:
...
class CompilerError(RuntimeError): pass
...
def _acceptance_for(kind: str, scope_paths: list[str]) -> list[str]:
    ...
    if kind == "testing":
        ...
        if has_py: a.append("pytest -x -q")
        elif has_js: a.append("npx jest --passWithNoTests")
...
def _infer_scope(analyses: list, objective: str) -> list[str]:
    stop = {"the","a","an","is","are","was","were","be","been",
            ...
            "at","by","from","as","through","during","before","after",
            "all","each","every","few","more","most","only","own",
            "same","some","such","not","no"}
    ...
    for fa in analyses:
        ...
        for sym in fa.symbols:
            for w in words:
                if w in sym.name.lower(): score += 3
                if w in (sym.signature or "").lower(): score += 1
        for imp in fa.imports:
            for w in words:
                if w in imp.lower(): score += 2
        if fa.test_functions and "test" not in words:
            score -= len(fa.test_functions) * 2
        if score > 0: scored.append((score, fa.path))
...
def _walk_ts(node, source, path, lang, symbols, depth=0) -> None:
    ...
    if node.type in kinds:
        ...
        symbols.append(Symbol(name=name or node.type, kind=node.type, path=path,
...
def analyze_codebase(cwd: str) -> list[FileAnalysis]:
    ...
    for root, dirs, files in os.walk(cwd_path):
        dirs[:] = [d for d in dirs if not d.startswith(".")
                   and d not in {"__pycache__","vendor","node_modules",
                                  "venv",".venv",".forgeos",".forgeos-live"}]
        for f in sorted(files):
            ext = Path(f).suffix.lower()
            if ext in TREE_EXTS:
                source_files.append(Path(root) / f)
            if len(source_files) >= MAX_SOURCE: break
        if len(source_files) >= MAX_SOURCE: break
    for sp in source_files[:100]:
        try:
            ...
            if syms:
                ...
                fa.imports = [s.signature for s in syms if s.kind in ("import_statement","import_declaration","require_statement")]
                fa.test_functions = [s for s in syms if any(m in s.name.lower() for m in ("test_","spec_","_test","_spec","should_"))]
                analyses.append(fa)
        except Exception:
            continue
    return analyses
...
def compile_mission(objective: str, *, cwd: str = ".", max_tasks: int = 6) -> Mission:
    ...
    if kind == "bugfix" and max_tasks >= 2:
        ...
        t2 = TaskSpec(id=new_id("task"), job_id="",
            ...
            scope=Scope(paths=scope[:5]), budget=_budget_for("refactor"),
            capabilities=["edit","refactor","python"], depends_on=[t1.id])
        tasks.append(t2); deps[t2.id] = [t1.id]
    else:
        tasks.append(TaskSpec(id=new_id("task"), job_id="",
            subject=objective[:150],
            description=f"Work on: {objective}. Scope: {' '.join(scope[:10])}",
            acceptance=_acceptance_for(kind, scope), scope=Scope(paths=scope[:10]),
            budget=_budget_for(kind), capabilities=_caps_for(kind)))

...
- Current audit of the concurrent control-plane work: `tests/test_automemory.py` passed 13; all 11 hook cases passed across the full run/isolation rerun; all 12 MCP stdio cases passed via checkpointed `sweep.py`. A separate full-suite runner is still active while additional `forgeos/core/probe.py` work is being added; do not claim the combined suite green yet.
